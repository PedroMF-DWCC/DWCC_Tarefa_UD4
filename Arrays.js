let paises = ["Argentina", "Brasil", "Canadá", "Dinamarca", "Egipto", "Francia", "Grecia", "España", "India", "Japón"];
let string = "";
...
paises.forEach((el, i) => {
    ...
    if(i === paises.length-1){string += `${paises[i]}.`;}
    else string += `${paises[i]}, `;
});
document.write(`<h5>${string}</h5>`);
...
const numUsuario = prompt("Introduce un número para ver que país se atopa en esa posición do array:");
...
//##### UD04 Exercicio 9 #####
let paisUsuario = prompt("Introduce o país do que queiras conocer a posición:");
paisUsuario = paisUsuario.toLowerCase();

const expLowCase = /^[a-záéíóúüñ]+$/;
...
if(expLowCase.test(paisUsuario)){
    ...
    if(expLowCase.test(paisUsuario) && paisesLowCase.indexOf(paisUsuario) != -1){
        ...
        document.write(`<h3>9. O país ${paisUsuario} atópase na posición: ${paisesLowCase.indexOf(paisUsuario)}`);
    }else document.write(`<h3>9. O país ${paisUsuario} non se atopa no array</h3>`);

}else document.write(`<h3>9. O nome do país introducido non é valido (${paisUsuario})`);


//##### UD04 Exercicio 10 #####
string = "";

let inicioRangoUsuario = prompt("Indica o primeiro elemento do rango a mostrar (0-9): ");
let finalRangoUsuario = prompt("Indica o último elemento do rango a mostrar (0-9): ");
let nuevoArray = [];
...
if(inicioRangoUsuario >= 0 && inicioRangoUsuario <= 9 && finalRangoUsuario >= 0 && finalRangoUsuario <= 9 && inicioRangoUsuario <= finalRangoUsuario){
    ...
    nuevoArray.forEach((el, i) => {
        ...
        if(i === nuevoArray.length-1){string += `${nuevoArray[i]}.`;}
        else string += `${nuevoArray[i]}, `;
    });
    document.write(`<h5>${string}</h5>`);
}else document.write(`<h3>10. Rangos introducidos non admitidos</h3>`);
